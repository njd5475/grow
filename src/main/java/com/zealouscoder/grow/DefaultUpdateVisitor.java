...
public class DefaultUpdateVisitor implements UpdateVisitor {
	...
	public void update(double dt, LifeCell lifeCell) {
		if (lifeCell.isSpawnTime(dt)) {
			Point pt = lifeCell.getNextSpawnPoint();
			CellType type = lifeCell.getNextSpawnType();
			try {
				if (game.is(pt.x, pt.y, CellType.EMPTY)) {
					game.swapFor(type, pt.x, pt.y);
				}
